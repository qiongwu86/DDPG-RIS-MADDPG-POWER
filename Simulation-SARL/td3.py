...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, fc1_dims, fc2_dims, n_actions, name,
        ...
        self.fc2_dims = fc2_dims
        self.n_actions = n_actions
        self.checkpoint_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)))
        self.checkpoint_file = os.path.join(chkpt_dir, name + '_TD3')
        self.fc1 = nn.Linear(self.input_dims, self.fc1_dims)
        f1 = 1. / np.sqrt(self.fc1.weight.data.size()[0])
        #        T.nn.init.uniform_(self.fc1.weight.data, -f1, f1)
...
class Agent_TD3(object):
    def __init__(self, alpha, beta, input_dims, tau, gamma=0.99,
                 n_actions=52, max_size=1000000, layer1_size=1024,
                 layer2_size=512, layer3_size=256, batch_size=64,
                 update_actor_interval=2, noise='OU'):
        ...
        self.actor = ActorNetwork(alpha, input_dims, self.layer2_size,
                                  self.layer3_size, n_actions=n_actions,
                                  name='Actor')
        self.critic_1 = CriticNetwork(beta, input_dims, self.layer1_size,
                                      self.layer2_size, self.layer3_size, n_actions=n_actions,
                                      name='Critic_1')
        self.critic_2 = CriticNetwork(beta, input_dims, self.layer1_size,
                                      self.layer2_size, self.layer3_size, n_actions=n_actions,
                                      name='Critic_2')

        self.target_actor = ActorNetwork(alpha, input_dims, self.layer2_size,
                                      self.layer3_size, n_actions=n_actions,
                                         name='TargetActor')
        self.target_critic_1 = CriticNetwork(beta, input_dims, self.layer1_size,
                                      self.layer2_size, self.layer3_size, n_actions=n_actions,
                                             name='TargetCritic_1')
        self.target_critic_2 = CriticNetwork(beta, input_dims, self.layer1_size,
                                      self.layer2_size, self.layer3_size, n_actions=n_actions,
                                             name='TargetCritic_2')
        if noise == 'OU':
