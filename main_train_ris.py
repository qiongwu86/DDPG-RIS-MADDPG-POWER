...
def get_state(idx):
    ...
    sinr_ris = env.compute_sinr_ris(idx, action_power) / 10000
...
if IS_TRAIN:
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(n_step_per_episode):
            ...
            '''per_user_reward, global_reward, databuf, data_trans, \
            data_local, over_power, over_data = env.step2(action_power)'''
        ...
        #输出全局奖励
        Sum_Power.append(np.mean(Power))
        Sum_Power_local.append(np.mean(Power_local))
        Sum_Power_offload.append(np.mean(Power_offload))
        ...
        print('Average Total Power:', np.mean(Sum_Power))
        print('Average local power:', np.mean(Sum_Power_local), '          Average offload power:',
    ...
    # 记录数据，绘制奖励函数曲线
    np.save('Data2/SARL_MARL3/3-User0_Reward_1000.npy', y2)
    np.save('Data2/SARL_MARL3/3-User1_Reward_1000.npy', y3)
    np.save('Data2/SARL_MARL3/3-User2_Reward_1000.npy', y4)
    np.save('Data2/SARL_MARL3/3-User3_Reward_1000.npy', y5)
    np.save('Data2/SARL_MARL3/3-User4_Reward_1000.npy', y6)
    np.save('Data2/SARL_MARL3/3-User5_Reward_1000.npy', y7)
    np.save('Data2/SARL_MARL3/3-User6_Reward_1000.npy', y8)
    np.save('Data2/SARL_MARL3/3-User7_Reward_1000.npy', y9)
    np.save('Data2/SARL_MARL3/3-Reward_MADDPG_1000.npy', record_global_reward_average)
